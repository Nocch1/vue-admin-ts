<template>
    <div class="app-container">
        <div class="filter-container">
            <el-input v-model="listQuery.title" :placeholder="t('table.title')" class="filter-item" style="width: 200px"
                      @keyup.enter="handleFilter" />
            <el-select v-model="listQuery.importance" :placeholder="t('table.importance')" class="filter-item" clearable style="width: 120px">
                <el-option v-for="item in importanceOptions" :key="item" :label="item" :value="item" />
            </el-select>
            <el-select v-model="listQuery.type" :placeholder="t('table.type')" class="filter-item" clearable style="width: 130px">
            ...
            <el-button v-waves :loading="downloadLoading" class="filter-item" icon="el-icon-download" type="primary" @click="handleDownload">
                {{ t('table.export') }}
            </el-button>
        ...
        <el-table :key="tableKey" v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%" @sort-change="sortChange">
            <el-table-column :class-name="getSortClass('id')" :label="t('table.id')" align="center" prop="id" sortable="custom" width="80">
                <template #default="{row}">
                    <span>{{ row.id }}</span>
                </template>
            </el-table-column>
            <el-table-column :label="t('table.date')" align="center" width="180px">
                <template #default="{row}">
            ...
            <el-table-column :label="t('table.author')" align="center" width="180px">
                <template #default="{row}">
                    <span>{{ row.author }}</span>
                </template>
            </el-table-column>
            <el-table-column v-if="showReviewer" :label="t('table.reviewer')" align="center" width="110px">
                <template #default="{row}">
            ...
            <el-table-column :label="t('table.importance')" width="105px">
                <template #default="{row}">
                    <svg-icon v-for="n in +row.importance" :key="n" class="iconfont iconxing" name="star" />
                </template>
            </el-table-column>
            ...
            <el-table-column :label="t('table.actions')" align="center" class-name="fixed-width" width="230">
        ...
        <el-pagination v-show="total > 0" v-model:limit="listQuery.limit" v-model:page="listQuery.page" :current-page="currentPage4"
                       :page-sizes="[10, 20, 50, 100]" :total="total" layout="total, sizes, prev, pager, next, jumper" @size-change="handleSizeChange"
                       @current-change="handleCurrentChange" />

        <el-dialog v-model="dialogFormVisible" :title="textMap[dialogStatus]">
            <el-form ref="dataForm" :model="tempArticleModel" :rules="rules" label-position="left" label-width="100px"
                ...
                <el-form-item :label="t('table.date')" prop="timestamp">
                    <el-date-picker v-model="tempArticleModel.timestamp" placeholder="Please pick a date" type="datetime" />
                </el-form-item>
                ...
                <el-form-item :label="t('table.importance')">
                    <el-rate v-model="tempArticleModel.importance" :colors="['#99A9BF', '#F7BA2A', '#FF9900']" :max="3" style="margin-top: 8px" />
                </el-form-item>
                <el-form-item :label="t('table.remark')">
                    <el-input v-model="tempArticleModel.abstractContent" :autosize="{minRows: 2, maxRows: 4}" placeholder="Please input"
                              type="textarea" />
                </el-form-item>
            </el-form>
...
import { createArticle, defaultArticleModel, getArticles, getPageviews, updateArticle } from '@/apis/articles';
